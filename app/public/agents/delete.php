...
if (!isset($_GET['id'])) {
  redirect_to(url_for('public/agents/index.php'));
}
$id = $_GET['id'];
$agents = Agent::find_by_id($id);
if ($agents == false) {
  redirect_to(url_for('public/agents/index.php'));
}
...
if (is_post_request()) {
  ...
  // logfile
  log_action('Delete agents', "id: {$agents->id}, Deleted by {$loggedInAdmin->full_name()}", "agents");

  // Delete agents
  $result = $agents->deleted($id);
  $session->message('The agents was deleted successfully.');
  redirect_to(url_for('public/agents/index.php'));
} else {
  // Display form
}
...
<link href="<?php echo url_for('assets/css/vendor/responsive.bootstrap5.css') ?>" rel="stylesheet" type="text/css" />
    ...
    <div class="row">
         <div class="card text-center p-4">
            ...
            <p class="item"><?php echo h($agents->first_name); ?></p>

            <form action="<?php echo url_for('public/agents/delete.php?id=' . h(u($id))); ?>" method="post">
